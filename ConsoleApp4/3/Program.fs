...
let parseNum = 
    float_ws |>> Num  // Парсит строку, что является числом
    
implementation := parseNum <|> parseOp// основные элементы, с которыми работает парсер

let rec EvalExpr(e:Expr):float =   // считает полученный алгебраический тип
    match e with
    | Num(num) -> num
    | Plus(a,b) ->
        let left =
...
let main argv =
    ...
    let expr1Parser = run parseExpression input
